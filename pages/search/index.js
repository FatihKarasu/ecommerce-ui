...
export default function index({
  ...
  const getQuery = () => {
    let query = `?query=${_query}&start=${start}&end=${end}&orderBy=${orderBy}&`;
  ...
  return (
    <>
      <Head>
        <title>{`Search - ${getTitle(_query)}`}</title>
...
export async function getServerSideProps(context) {
  ...
  let query = `?query=${context.query.query}&start=${start}&end=${end}&orderBy=${orderBy}&`;
  if (filterValues.categoryId !== null) {
